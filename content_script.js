class ExtManager {
    static sendToPopup(message, expectedResponse) {
...
class UIHandler {
    constructor() {
        ...
        this.button = document.getElementById("button_commentHackerScript");
        this.styleElem = null;
    ...
    colorsAccAppMode() {
        if (gloVar.isDarkMode == false) {
            ...
            DOMElems.controlsDiv.style.setProperty(
            ...
            DOMElems.button.style.boxShadow =
            ...
            DOMElems.cmntsCountDiv.style.boxShadow =
                "rgba(255, 255, 255, 0.9) -4px -4px 7px 1px, rgba(0, 0, 0, 0.3) 4px 4px 13px";
...
const methods = {
    toggleButtonState: () => {
        ...
        setTimeout(() => {
            if (DOMElems.button.textContent == "Start") {
                DOMElems.button.parentNode.style.setProperty(
                    "--bubbles_color",
                    "#20f020"
                );
            } else {
        ...
        if (!DOMElems.controlsDiv) {
            ...
            methods.fetchFiles("assets/commentBoxUi.css").then((css) => {
                ...
                methods.fetchFiles("assets/commentBoxUi.html").then((html) => {
                    ...
                    // Updating the DOMElems object
                    DOMElems = {
                        controlsDiv: document.getElementById(
                            "controlsDiv_commentHackerScript"
                        ),
                        ...
                        cmntsCountDiv: document.getElementById(
                            "cmntsCountDiv_commentHackerScript"
                        ),
                    };
    ...
    toggleStartCommenting: () => {
        const commentMethods = {
            ...
            runRandomInterval: () => {
                let intervalTime =
                    Math.floor(
                        Math.random() * (gloVar.maxTime - gloVar.minTime)
                    ) + gloVar.minTime; // random interval time between minTime and maxTime
                ...
                fetch(
                    ...
                    .then((commentsTextStack) => {
                        ...
                        let finalText =
                            commentsTextStack[
                                Math.floor(
                                    Math.random() * commentsTextStack.length
                                )
                            ] +
                            " " +
                            emojiString;
